...
runResult="$(__sushi_process_run 'git' "$(__sushi_json_array '--version')" '' '' '' 0 'true' 'false')"
printf '%s\n' 'git --version:'
printf '%s\n' "$(__sushi_json_member "${runResult:-}" 'stdout')"
printf '%s\n' 'git exit code:'
printf '%s\n' "$(__sushi_json_member "${runResult:-}" 'code')"
printf '%s\n' 'git stderr:'
printf '%s\n' "$(__sushi_json_member "${runResult:-}" 'stderr')"
stages="$(__sushi_json_array "$(__sushi_json_object 'command' 'git' 'args' "$(__sushi_json_array 'status' '--short')")" "$(__sushi_json_object 'command' 'git' 'args' "$(__sushi_json_array 'hash-object' '--stdin')")")"
pipelineResult="$(__sushi_process_pipeline "${stages:-}" '' '' '' 0 'true' 'false')"
printf '%s\n' 'pipeline stdout:'
printf '%s\n' "$(__sushi_json_member "${pipelineResult:-}" 'stdout')"
printf '%s\n' 'pipeline exit code:'
printf '%s\n' "$(__sushi_json_member "${pipelineResult:-}" 'code')"
printf '%s\n' 'pipeline stderr:'
