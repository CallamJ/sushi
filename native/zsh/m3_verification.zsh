...
failures=0
printf '%s\n' 'M3 verification started'
toolName='dotnet'
runArgs="$(__sushi_json_array '--version')"
pipelineStages="$(__sushi_json_array "$(__sushi_json_object 'command' 'dotnet' 'args' "$(__sushi_json_array '--version')")" "$(__sushi_json_object 'command' 'dotnet' 'args' "$(__sushi_json_array '--info')")")"
runResult="$(__sushi_process_run "${toolName:-}" "${runArgs:-}" '' '' '' 0 'true' 'false')"
if [[ "$(__sushi_json_member "${runResult:-}" 'code')" != 0 ]]; then
    printf '%s\n' 'dotnet unavailable, trying git fallback'
    toolName='git'
    runArgs="$(__sushi_json_array '--version')"
    pipelineStages="$(__sushi_json_array "$(__sushi_json_object 'command' 'git' 'args' "$(__sushi_json_array '--version')")" "$(__sushi_json_object 'command' 'git' 'args' "$(__sushi_json_array 'hash-object' '--stdin')")")"
    runResult="$(__sushi_process_run "${toolName:-}" "${runArgs:-}" '' '' '' 0 'true' 'false')"
fi
if [[ "$(__sushi_json_member "${runResult:-}" 'code')" == 0 ]]; then
    printf '%s\n' 'PASS process.run exit code'
else
    printf '%s\n' 'FAIL process.run exit code'
    printf '%s\n' "$(__sushi_json_member "${runResult:-}" 'stderr')"
    failures=$(( ${failures:-0} + 1 ))
fi
if [[ "$(__sushi_json_member "${runResult:-}" 'stdout')" != '' ]]; then
    printf '%s\n' 'PASS process.run stdout'
else
    printf '%s\n' 'FAIL process.run stdout'
    failures=$(( ${failures:-0} + 1 ))
fi
pipelineResult="$(__sushi_process_pipeline "${pipelineStages:-}" '' '' '' 0 'true' 'false')"
if [[ "$(__sushi_json_member "${pipelineResult:-}" 'code')" == 0 ]]; then
    printf '%s\n' 'PASS process.pipeline exit code'
else
    printf '%s\n' 'FAIL process.pipeline exit code'
    printf '%s\n' "$(__sushi_json_member "${pipelineResult:-}" 'stderr')"
    failures=$(( ${failures:-0} + 1 ))
fi
if [[ "$(__sushi_json_member "${pipelineResult:-}" 'stdout')" != '' ]]; then
    printf '%s\n' 'PASS process.pipeline stdout'
else
    printf '%s\n' 'FAIL process.pipeline stdout'
    failures=$(( ${failures:-0} + 1 ))
fi
payload="$(__sushi_json_object 'tool' 'sushi' 'count' 3 'ok' 'true')"
jsonText="$(__sushi_json_stringify "${payload:-}" 0)"
parsed="$(__sushi_json_parse "${jsonText:-}")"
if [[ "$(__sushi_json_member "${parsed:-}" 'tool')" == 'sushi' ]]; then
    printf '%s\n' 'PASS json.parse field'
else
    printf '%s\n' 'FAIL json.parse field'
    failures=$(( ${failures:-0} + 1 ))
fi
if [[ "$(__sushi_json_member "${parsed:-}" 'count')" == 3 ]]; then
    printf '%s\n' 'PASS json.parse numeric field'
else
    printf '%s\n' 'FAIL json.parse numeric field'
    failures=$(( ${failures:-0} + 1 ))
fi
if [[ "$(__sushi_json_member "${parsed:-}" 'ok')" == 'true' ]]; then
    printf '%s\n' 'PASS json.parse bool field'
else
    printf '%s\n' 'FAIL json.parse bool field'
    failures=$(( ${failures:-0} + 1 ))
fi
verifyPath=$(printf '%s' 'tmp'; printf '/%s' 'm3_verify.txt')
__sushi_path=$(printf '%s' "${verifyPath:-}"); __sushi_dir=$(dirname -- "$__sushi_path"); if [[ "$__sushi_dir" != "." && ! -d "$__sushi_dir" ]]; then mkdir -p -- "$__sushi_dir"; fi; if [[ 'false' == 'true' ]]; then printf '%s' 'ok' >> "$__sushi_path"; else printf '%s' 'ok' > "$__sushi_path"; fi
globMatches="$(__sushi_fs_glob 'tmp/m3_verify*.txt' '')"
firstMatch="$(__sushi_json_index "${globMatches:-}" 0)"
if [[ "${firstMatch:-}" != '' ]]; then
    printf '%s\n' 'PASS fs.glob'
else
    printf '%s\n' 'FAIL fs.glob'
...
if [[ "${skipHttp:-}" == '1' ]]; then
    ...
    response="$(__sushi_http_get 'https://example.com' '')"
    if [[ "$(__sushi_json_member "${response:-}" 'status')" == 0 ]]; then
        printf '%s\n' 'SKIP http (unavailable in current runtime/network)'
    else
        if [[ "$(__sushi_json_member "${response:-}" 'ok')" == 'true' ]]; then
            printf '%s\n' 'PASS http.get ok'
        else
            printf '%s\n' 'FAIL http.get ok'
            printf '%s\n' "$(__sushi_json_member "${response:-}" 'status')"
            failures=$(( ${failures:-0} + 1 ))
        fi
        if [[ "$(__sushi_json_member "${response:-}" 'body')" != '' ]]; then
            printf '%s\n' 'PASS http.get body'
        else
            printf '%s\n' 'FAIL http.get body'
            failures=$(( ${failures:-0} + 1 ))
