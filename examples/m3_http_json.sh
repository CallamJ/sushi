...
response="$(__sushi_http_get 'https://api.github.com' '')"
printf '%s\n' 'status:'
printf '%s\n' "$(__sushi_json_member "${response:-}" 'status')"
if [[ "$(__sushi_json_member "${response:-}" 'ok')" != '' ]]; then
    printf '%s\n' 'headers:'
    printf '%s\n' "$(__sushi_json_stringify "$(__sushi_json_member "${response:-}" 'headers')" 2)"
    if [[ "$(__sushi_json_member "${response:-}" 'json')" != '' ]]; then
        printf '%s\n' 'json body preview:'
        printf '%s\n' "$(__sushi_json_stringify "$(__sushi_json_member "${response:-}" 'json')" 2)"
    else
        printf '%s\n' 'raw body preview:'
        printf '%s\n' "$(__sushi_json_member "${response:-}" 'body')"
    fi
fi
