...
sum() {
    local -a __sushi_varargs_values=("${@:1}")
    local values="$(__sushi_json_array "${__sushi_varargs_values[@]}")"
    local __sushi_vararg_item
    while IFS= read -r __sushi_vararg_item; do
        __sushi_type_check "${__sushi_vararg_item:-}" 'int' 'varargs parameter '"'"'values'"'"' of function '"'"'sum'"'"'' || exit 2
    done < <(__sushi_json_array_each_raw "${values:-[]}")
    total=0
    i=0
    while (( $(__sushi_require_integer "${i:-}" 'variable '"'"'i'"'"'') < 4 )); do
        total=$(( ($(__sushi_require_integer "${total:-}" 'variable '"'"'total'"'"'') + $(__sushi_require_integer "$(__sushi_json_index "${values:-}" "${i:-}")" 'arithmetic operand')) ))
        i=$(( ($(__sushi_require_integer "${i:-}" 'variable '"'"'i'"'"'') + 1) ))
    done
    printf '%s\n' "${total:-}"
    return 0
}
